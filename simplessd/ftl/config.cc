...
const char NAME_REFRESH_MODE[] = "RefreshMode";
const char NAME_REFRESH_MAX_LAYER_NUM[] = "RefreshMaxLayerNum";
const char NAME_REFRESH_GROUPING_SIZE[] = "RefreshGroupingSize";
...
const char NAME_GC_RECO_PARAM[] = "RecoGCParam";

const char NAME_HOT_COLD_SEPARATION[] = "HotColdSeparation";
const char NAME_HOT_BLOCK_RATIO[] = "HotBlockRatio";
const char NAME_COOL_DOWN_WINDOW_SIZE[] = "CoolDownWindowSize";
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_REFRESH_MODE)) {
    refreshMode = strtoul(value, nullptr, 10);
  }
  else if (MATCH_NAME(NAME_REFRESH_MAX_LAYER_NUM)) {
    refreshMaxLayerNum = strtoul(value, nullptr, 10);
  }
  else if (MATCH_NAME(NAME_REFRESH_GROUPING_SIZE)) {
    refreshGroupingSize = strtoul(value, nullptr, 10);
  }
  ...
  else if (MATCH_NAME(NAME_GC_RECO_PARAM)) {
    recoGCParam = strtof(value, nullptr);
  }
  else if (MATCH_NAME(NAME_HOT_COLD_SEPARATION)) {
    hotColdSeperation = strtoul(value, nullptr, 10);
  }
  else if (MATCH_NAME(NAME_HOT_BLOCK_RATIO)) {
    hotBlocksRatio = strtof(value, nullptr);
  }
  else if (MATCH_NAME(NAME_COOL_DOWN_WINDOW_SIZE)) {
    coolDownWindowSize = strtoul(value, nullptr, 10);
  }
...
uint64_t Config::readUint(uint32_t idx) {
  ...
  switch (idx) {
    ...
    case FTL_REFRESH_FILTER_SIZE:
      ...
      break;
    case FTL_REFRESH_MODE:
      ret = refreshMode;
      break;
    case FTL_REFRESH_MAX_LAYER_NUM:
      ret = refreshMaxLayerNum;
      break;
    case FTL_REFRESH_GROUPING_SIZE:
      ret = refreshGroupingSize;
      break;
    case FTL_HOT_COLD_SEPERATION :
      ret = hotColdSeperation;
      break;
    case FTL_COOL_DOWN_WINDOW_SIZE:
      ret = coolDownWindowSize;
      break;
  }
...
float Config::readFloat(uint32_t idx) {
  ...
  switch (idx) {
    ...
    case FTL_REFRESH_MAX_RBER:
      ret = refreshMaxRBER;
      break;
    case FTL_GC_RECO_PARAM:
      ret = recoGCParam;
      break;
    case FTL_HOT_BLOCK_RATIO:
      ret = hotBlocksRatio;
      break;
  }
