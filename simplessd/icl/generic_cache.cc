...
void GenericCache::getStatList(std::vector<Stats> &list, std::string prefix) {
  //debugprint(LOG_ICL_GENERIC_CACHE, "get stat list start");
  Stats temp;
  temp.name = prefix;
  list.push_back(temp);
  list.pop_back();
  //debugprint(LOG_ICL_GENERIC_CACHE, "get stat list end");
  /*
...
void GenericCache::getStatValues(std::vector<double> &values) {
  //debugprint(LOG_ICL_GENERIC_CACHE, "get stat values start");
  values.push_back(0);
  values.pop_back();
  //debugprint(LOG_ICL_GENERIC_CACHE, "get stat values end");
