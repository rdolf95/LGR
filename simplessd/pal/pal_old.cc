...
void PALOLD::getStatList(std::vector<Stats> &list, std::string prefix) {
  //debugprint(LOG_PAL_OLD, "get stat list start");
  Stats temp;
  ...
  */
  ...
  list.push_back(temp);
  */
 //debugprint(LOG_PAL_OLD, "get stat list end");
}

void PALOLD::getStatValues(std::vector<double> &values) {
  //debugprint(LOG_PAL_OLD, "get stat values start");
  ...
  //values.push_back(active.average);
  //debugprint(LOG_PAL_OLD, "get stat values end");
}
